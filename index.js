...
store.bind(bara.ev);
bara.ev.on("messages.upsert", async (chatUpdate, msg) => {
 try {
const mek = chatUpdate.messages[0]
if (!mek.message) return
mek.message = (Object.keys(mek.message)[0] === 'ephemeralMessage') ? mek.message.ephemeralMessage.message : mek.message
if (mek.key && mek.key.remoteJid === 'status@broadcast') return
if (!bara.public && !mek.key.fromMe && chatUpdate.type === 'notify') return
if (mek.key.id.startsWith('BAE5') && mek.key.id.length === 16) return
if (mek.key.id.startsWith('BaraSetiawan')) return;
const m = smsg(bara, mek, store)
require("./lib/Button")(bara, m, chatUpdate, store)
 } catch (err) {
 console.log(err)
 }
});
    ...
    bara.sendText = (jid, text, quoted = '', options) => bara.sendMessage(jid, { text: text, ...options }, { quoted });
    
    bara.downloadMediaMessage = async (message) => {
